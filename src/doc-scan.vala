...
public class DocScan : Gtk.Application
{
    ...
    public static const OptionEntry[] options =
    {
        ...
        { "fix-pdf", 0, 0, OptionArg.STRING, ref fix_pdf_filename,
          N_("Fix PDF files generated with older versions of Doc Scan"), "FILENAME..."},
        { null }
    };
    ...
    public DocScan (ScanDevice? device = null)
    {
    ...
    public static int main (string[] args)
    {
        ...
        /* Log to a file */
        log_timer = new Timer ();
        var path = Path.build_filename (Environment.get_user_cache_dir (), "doc-scan", null);
        DirUtils.create_with_parents (path, 0700);
        path = Path.build_filename (Environment.get_user_cache_dir (), "doc-scan", "doc-scan.log", null);
        log_file = FileStream.open (path, "w");
        Log.set_default_handler (log_cb);

        debug ("Starting Doc Scan %s, PID=%i", VERSION, Posix.getpid ());

        var app = new DocScan (device);
        return app.run ();
    }
}
